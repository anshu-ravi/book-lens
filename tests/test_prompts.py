...
VALID_DIGEST_MD = (
    "---\nseq: 5 | book: invented | label: \"Ch 1\" | pov: someone | location: nowhere\n"
    "entities: []\nintroduces: []\nsource_paras: [1, 2]\n---\n"
    "## Events\n- invented event\n"
    "## State changes\n- invented change\n"
    "## Open questions\n- invented question\n"
...
def test_build_rollup_prompt_never_needs_raw_paragraphs():
    bundle = prompts.build_rollup_prompt(
        book_id="b1", level="part", target_label="Part One", source_digests=[VALID_DIGEST_MD]
    )
    ...
    assert parsed["source_digests"] == [VALID_DIGEST_MD]
...
def test_parse_rollup_response_unwraps_markdown_code_fence():
    fenced = "```json\n" + json.dumps({"digest_markdown": VALID_DIGEST_MD}) + "\n```"
    assert prompts.parse_rollup_response(fenced) == VALID_DIGEST_MD
...
def test_parse_rollup_response_happy_path():
    text = prompts.parse_rollup_response(json.dumps({"digest_markdown": VALID_DIGEST_MD}))
    assert text == VALID_DIGEST_MD
...
def test_parse_rollup_response_rejects_missing_section():
    bad_md = VALID_DIGEST_MD.replace("## Events", "## Stuff")
    with pytest.raises(ValueError):
