...
class ExtractionService:
    ...
    async def extract_book(
        ...
        try:
            ...
            # Write flat chunks to book_chunks table (legacy; kept for backfill scripts).
            # The active vector-search path now uses book_rag_nodes (see backend/rag/).
            # Run scripts/run_rag_ingest.py after extraction to populate those tables.
            try:
                ...
                logger.info(f"Stored {n_chunks} legacy text chunks for book {self.book_id}")
    ...
    async def _trigger_downstream_stages(self) -> None:
        ...
        try:
            from backend.knowledge.deduplication_service import DeduplicationService
